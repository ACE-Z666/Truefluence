...
MULTIMODALS    = os.path.join(ROOT_DIR, 'core-ai')
FRONTEND_DIR   = os.path.join(ROOT_DIR, 'apps', 'web-dashboard')
UPLOAD_DIR     = os.path.join(ROOT_DIR, 'server', 'uploads')
REELS_DB       = os.path.join(ROOT_DIR, 'server', 'reels_db.json')
...
import torch
from src.video_engine import VisualQualityHead, extract_quality_frames
from src.audio_engine   import AdvancedAudioAnalyzer
from train_pipeline          import AudioClassificationHead, extract_visual_features, extract_audio_features
from src.mesonet        import load_meso4, screen_for_deepfake
...
WEIGHTS_PATH      = os.path.join(MULTIMODALS, 'weights', 'best_model.pth')
MESO_WEIGHTS      = os.path.join(MULTIMODALS, 'weights', 'meso4_DF.pth')
